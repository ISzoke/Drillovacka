...
const BADGE_KEY_MAP = {
    first_correct: 'badgeFirstCorrect', ten_correct: 'badgeTenCorrect', hundred_correct: 'badgeHundredCorrect',
    three_hundred_correct: 'badgeThreeHundredCorrect', five_hundred_correct: 'badgeFiveHundredCorrect', thousand_correct: 'badgeThousandCorrect',
    five_skills: 'badgeFiveSkills', ten_skills: 'badgeTenSkills', twenty_skills: 'badgeTwentySkills',
    comeback: 'badgeComeback', ten_in_a_row: 'badgeTenInARow', twenty_five_in_a_row: 'badgeTwentyFiveInARow',
    perfect_session: 'badgePerfectSession', accuracy_master: 'badgeAccuracyMaster', accuracy_legend: 'badgeAccuracyLegend',
    mastery_skill: 'badgeMasterySkill', fast_answer: 'badgeFastAnswer', lightning: 'badgeLightning',
    speed_demon: 'badgeSpeedDemon', speed_session: 'badgeSpeedSession',
    streak_3: 'badgeStreak3', streak_7: 'badgeStreak7', streak_14: 'badgeStreak14', streak_30: 'badgeStreak30',
    level_5: 'badgeLevel5', level_10: 'badgeLevel10', level_20: 'badgeLevel20',
    voice_first: 'badgeVoiceFirst', voice_10: 'badgeVoice10', voice_50: 'badgeVoice50',
    voice_100: 'badgeVoice100', voice_250: 'badgeVoice250',
    top10_leaderboard: 'badgeTop10Leaderboard', top3_leaderboard: 'badgeTop3Leaderboard',
};

const getBadgeLabel = (bkey) => BADGE_KEY_MAP[bkey] ? t(BADGE_KEY_MAP[bkey]) : bkey;
...
const congratsMessage = computed(() => {
    ...
    if (faster && more) return t('congratsBothImproved');
    if (faster)         return t('congratsFasterOnly');
    if (more)           return t('congratsMoreOnly');
    return '';
});
...
<template>
    <div class="flex flex-col items-center justify-center mt-8 md:mt-16 rounded-3xl
        ...
        <h2 class="text-lg md:text-2xl font-bold text-slate-600 dark:text-slate-300 text-center mt-4">
            {{ t('exampleCountText') }} {{ total }} {{ getCorrectForm }}
        </h2>
        ...
        <div class="grid grid-cols-1 md:flex md:justify-center md:space-x-4 mt-4 text-base font-medium md:text-sm">
            <div v-if="props.skipped      > 0" class="flex items-center space-x-2">
                ...
                <span>{{ t('skipped')      }} ({{ props.skipped }})</span>
            </div>
            <div v-if="props.noMistakes   > 0" class="flex items-center space-x-2">
                ...
                <span>{{ t('noMistakes')   }} ({{ props.noMistakes }})</span>
            </div>
            <div v-if="props.oneMistake   > 0" class="flex items-center space-x-2">
                ...
                <span>{{ t('oneMistake')   }} ({{ props.oneMistake }})</span>
            </div>
            <div v-if="props.twoMistakes  > 0" class="flex items-center space-x-2">
                ...
                <span>{{ t('twoMistakes')  }} ({{ props.twoMistakes }})</span>
            </div>
            <div v-if="props.threeMistakes> 0" class="flex items-center space-x-2">
                ...
                <span>{{ t('threeMistakes')}} ({{ props.threeMistakes }})</span>
            </div>
        </div>
        ...
        <div v-if="currSession" class="w-full md:max-w-lg mt-8 flex flex-col gap-3">
                ...
                <div v-if="improved"
                    ...
                    <div>
                        ...
                        <div v-if="timeDeltaMs !== null && timeDeltaMs < 0"
                            ...
                            {{ t('previousSessionTime') }}: {{ fmtTime(prevSession.avg_time) }}
                        </div>
                    </div>
                </div>
            </Transition>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div class="bg-slate-50 dark:bg-slate-900 rounded-2xl px-4 py-4 flex flex-col justify-between min-h-[100px]">
                    <div class="text-xs font-bold text-slate-400 dark:text-slate-500 mb-1">⏱ {{ t('avgTimeLabel') }}</div>
                    ...
                    <div v-if="timeDeltaMs !== null"
                        ...
                        <span class="font-semibold opacity-70">
                            {{ timeDeltaMs < 0 ? t('fasterTime') : t('slowerTime') }}
                        </span>
                    </div>
                    <div v-else class="text-xs text-slate-300 dark:text-slate-600 mt-2">{{ t('firstSession') }}</div>
                </div>

                <!-- Mastery tile -->
                <div class="bg-slate-50 dark:bg-slate-900 rounded-2xl px-4 py-4 flex flex-col justify-between min-h-[100px]">
                    <div class="text-xs font-bold text-slate-400 dark:text-slate-500 mb-1">📈 {{ t('mastery') }}</div>
                    ...
                    <div v-if="masteryDelta !== null"
                        ...
                        <span v-else class="font-semibold opacity-70">{{ t('unchangedMastery') }}</span>
                    </div>
                    <div v-else class="text-xs text-slate-300 dark:text-slate-600 mt-2">{{ t('firstSession') }}</div>
                </div>
            </div>
        ...
        <div v-if="authStore.isAuthenticated && authStore.role !== 'admin'"
            ...
            <div v-if="gamStore.xp > 0"
                ...
                <span class="text-3xl">⭐</span>
                <div>
                    <div class="font-bold text-violet-700 text-lg">{{ t('level') }} {{ gamStore.level }}</div>
                    <div class="text-sm text-violet-500">{{ gamStore.xp }} {{ t('xpTotalSuffix') }} · 🔥 {{ gamStore.streak }} {{ t('days') }}</div>
                </div>
            </div>
