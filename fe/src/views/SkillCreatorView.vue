...
const confirmDelete = async () => {
  try {
    ...
    toastStore.addToast({
      message: t('skillDeletedToast', { name: deletedSkillName.value }),
      type: 'success',
      visible: true,
    });
...
const getCorrectFormOfTask = (count) => {
  if (count === 1) {
    return t('skillFormTaskSingular');
  } else  {
    return t('skillFormTaskPlural');
  }
};
...
const getCorrectFormOfExample = (count) => {
  if (count === 1) {
    return t('skillFormExampleSingular');
  } else  {
    return t('skillFormExamplePlural');
  }
};
</script>
...
<template>
  ...
  <div class="flex flex-col w-full max-w-4xl mx-auto pt-12">
    <h2 class="text-4xl font-bold text-primary my-4 text-center">{{ t('skills') }}</h2>
  ...
  <div v-if="isModalOpened" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white p-6 rounded-lg shadow-lg w-2/3 md:w-1/2">
      <p class="text-2xl">{{ t('confirmDeleteSkill', { name: deletedSkillName }) }}</p>
      <p class="mt-2 text-lg text-gray-600">{{ t('skillLinkedInfo', { tasks: relatedTasks + ' ' + getCorrectFormOfTask(relatedTasks), examples: relatedExamples + ' ' + getCorrectFormOfExample(relatedExamples) }) }}</p>
      <p class="mt-2 text-lg font-semibold text-red-600"><i class="fa-solid fa-triangle-exclamation text-xl"></i> {{ t('actionIrreversible') }} </p>
      <div class="mt-4 flex justify-end gap-2 text-xl font-semibold">
        <button @click="closeModal" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400">{{ t('cancel') }}</button>
        <button @click="confirmDelete" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">{{ t('delete') }}</button>
      </div>
    </div>
  </div>
