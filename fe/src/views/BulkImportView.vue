...
const jsonPlaceholder = `[
  {
    ...
    "examples": [
      { "example": "3 + 4 =", "input_type": "INLINE", "answer": "7" },
      { "example": "6 + 2 =", "input_type": "INLINE", "answer": "8" }
    ]
  },
  {
    "task_name": "Odčítanie do 20",
    "examples": [
      { "example": "15 - 7 =", "input_type": "INLINE", "answer": "8" },
      { "example": "20 - 13 =", "input_type": "INLINE", "answer": "7" }
    ]
  }
]`;
...
const previewCount = computed(() => {
  ...
  const tasks = parsedTasks.value.length;
  const examples = parsedTasks.value.reduce(
    (sum, t) => sum + (t.examples || []).length, 0
  );
  return { tasks, examples };
});

const canSubmit = computed(() => {
  return parsedTasks.value && selectedSkillIds.value.length > 0 && !importing.value;
});
...
const handleImport = async () => {
  ...
  try {
    const tasks = parsedTasks.value.map((t) => ({
      ...
      skill_ids: t.skill_ids || selectedSkillIds.value,
      grade_ids: t.grade_ids || selectedGradeIds.value,
      examples: t.examples || [],
    }));
  ...
  <div class="max-w-5xl mx-auto px-4 pt-12 pb-16">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <p class="mt-2 text-slate-600">
          Vlož JSON s taskami a príkladmi, vyber zručnosti a ročníky, a importuj naraz.
        </p>
      </div>
    ...
    <div v-else class="mt-6 space-y-6">
      ...
      <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 class="text-lg font-bold text-slate-900">Zručnosti (spoločné pre všetky tasky)</h2>
        <p class="text-sm text-slate-500 mb-3">
          Vyber zručnosti, ktoré sa priradia ku všetkým importovaným taskom.
          Jednotlivé tasky v JSON môžu prepísať cez vlastné <code>skill_ids</code>.
        </p>
        <div class="flex flex-wrap gap-2">
          <button
            ...
            :style="{ marginLeft: skill.depth * 12 + 'px' }"
          >
            {{ skill.name }}
          </button>
        </div>
        <p v-if="selectedSkillIds.length === 0" class="mt-2 text-sm text-amber-600 font-medium">
          Vyber aspoň jednu zručnosť
        </p>
      </section>

      <!-- Grade selector -->
      <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 class="text-lg font-bold text-slate-900">Ročníky (voliteľné)</h2>
        <p class="text-sm text-slate-500 mb-3">
          Vyber ročníky, do ktorých sa tasky automaticky priradia.
        </p>
        <div class="flex flex-wrap gap-2">
          <button
      ...
      <!-- Form type -->
      <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 class="text-lg font-bold text-slate-900">Typ tasku (predvolený)</h2>
        <div class="flex gap-3 mt-3">
          <button
            type="button"
            @click="taskForm = 'classic'"
            class="rounded-lg border px-4 py-2 text-sm font-semibold transition"
            :class="taskForm === 'classic'
              ? 'border-primary bg-primary text-white'
          ...
          <button
            type="button"
            @click="taskForm = 'word-problem'"
            class="rounded-lg border px-4 py-2 text-sm font-semibold transition"
            :class="taskForm === 'word-problem'
              ? 'border-primary bg-primary text-white'
              : 'border-slate-300 bg-white text-slate-700'"
          >
            Word Problem
      ...
      <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p class="text-sm text-slate-500 mb-3">
          Vlož pole taskov. Každý task musí mať <code>task_name</code> a <code>examples</code>.
          Každý example musí mať <code>example</code>, <code>input_type</code> (INLINE/WORD/FRAC/VAR), a <code>answer</code>.
        </p>
        <textarea
          v-model="jsonInput"
      ...
      <div class="flex items-center gap-4">
        ...
        <a
          :href="'/api/export/csv/?all_actions=true'"
          target="_blank"
      ...
      <section v-if="results" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div class="space-y-2">
          ...
          >
            ...
            <span class="ml-auto">
              <template v-if="r.status === 'skipped'">
                Preskočené: {{ r.reason }}
              </template>
              <template v-else>
